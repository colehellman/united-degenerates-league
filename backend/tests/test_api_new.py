...
@pytest.mark.asyncio
async def test_list_competitions_optimized(
...
async def test_get_competition_not_found(client: AsyncClient, test_user: User):
    ...
    token = await _login(client)
    resp = await client.get(
        f"/api/competitions/{uuid.uuid4()}", headers={"Authorization": f"Bearer {token}"}
    )
...
async def test_delete_competition_forbidden(
    ...
    resp = await client.delete(
        f"/api/competitions/{active_competition.id}", headers={"Authorization": f"Bearer {token}"}
    )
...
async def test_get_leaderboard_sorting(
    ...
    # Sort by points
    resp = await client.get(
        f"/api/leaderboards/{active_competition.id}?sort_by=points",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200
